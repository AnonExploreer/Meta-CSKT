...
from ..core.evaluation import compute_shift_data, compute_shift_pre_data
...
class AnimalWeb(data.Dataset):
    ...
    def __init__(self, cfg, args, mode, pseudo_dict = None):
        ...
        if self.mode == 'unlabel':
            bad, good = {},{}
            self.hptsfile = args.hptsfile
            self.heightfile = args.heightfile
            self.fptsfile = args.fptsfile
            self.ffptsfile = args.ffptsfile
            with open(self.hptsfile) as json_file:
                self.hpts = json.load(json_file)
            with open(self.heightfile) as json_file:
                self.height = json.load(json_file)
            with open(self.fptsfile) as json_file:
                self.fpts = json.load(json_file)
            with open(self.ffptsfile) as json_file:
                self.ffpts = json.load(json_file)  
            ...
            print(args.f_threshold,len(self.landmarks_frame), l-len(self.landmarks_frame))
            print(len([x for x in self.landmarks_frame if self.p[x] == 1]))
