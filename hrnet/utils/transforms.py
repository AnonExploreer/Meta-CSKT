...
def fliplr_joints(x, width, dataset='aflw'):
    ...
    if dataset == 'WFLW' or dataset == 'animalweb':
...
#             img = scipy.misc.imresize(img, [new_ht, new_wd])  # (0-1)-->(0-255)
            img = np.array(Image.fromarray(np.uint8(img)).resize([new_wd,new_ht], PIL.Image.BICUBIC))
...
#     new_img = scipy.misc.imresize(new_img, output_size)
    new_img = np.array(Image.fromarray(np.uint8(new_img)).resize(output_size, PIL.Image.BICUBIC))
    return new_img
...
def generate_target_inverse(img, pt, sigma, label_type='Gaussian'):
    # Check that any part of the gaussian is in-bounds
    tmp_size = sigma * 3
    ul = [int(pt[0] - tmp_size), int(pt[1] - tmp_size)]
    br = [int(pt[0] + tmp_size + 1), int(pt[1] + tmp_size + 1)]
    ...
    img[img_x[0]:img_x[1], img_y[0]:img_y[1]] = g[g_x[0]:g_x[1], g_y[0]:g_y[1]]
    return img