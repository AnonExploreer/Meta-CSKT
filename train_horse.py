...
from hrnet.core.evaluation import compute_nme, decode_preds
from hrnet.utils.transforms import fliplr_joints, crop, generate_target_inverse, transform_pixel

from hrnet.data.animalwebV4_horse import AnimalWeb, dataloader_gen
...
parser.add_argument('--unlabel-root', type=str, required=True, help='data root of unlabel data')
parser.add_argument('--unlabel-bbox', type=str, required=True, help='bbox file of unlabel data')
...
def train_loop(args, labeled_loader, unlabeled_loader, test_loader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        try:
            images_uw, meta = unlabeled_iter.next()
        except:
            if args.world_size > 1:
                unlabeled_epoch += 1
                unlabeled_loader.sampler.set_epoch(unlabeled_epoch)
        ...
        if (step+1) % args.eval_step == 0:
            ...
            if args.local_rank in [-1, 0]:
                ...
                test_loss, nme,f8,f10 = evaluate(args, test_loader, test_model, criterion)
                ...
                logger.info(f"nme: {nme:.2f}, f0.08: {f8}, f0.1: {f10}")
    ...
    if args.local_rank in [-1, 0]:
        args.writer.add_scalar("result/test_nme", args.nme)
        wandb.log({"result/test_nme": args.nme})
    # finetune
...
def evaluate(args, test_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        test_iter.close()
        return losses.avg, nme,failure_008_rate ,failure_010_rate
