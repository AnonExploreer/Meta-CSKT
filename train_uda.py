...
def train_loop(cfg,args, labeled_loader, unlabeled_loader, test_loader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        try:
            images_uw, images_us ,meta, pseudo = unlabeled_iter.next()
        except:
            if args.world_size > 1:
                unlabeled_epoch += 1
                unlabeled_loader.sampler.set_epoch(unlabeled_epoch)
        ...
        with amp.autocast(enabled=args.amp):
            ...
            if True:
                maxidx = s_logits_uw.detach().view(s_logits_uw.size(0),s_logits_uw.size(1),-1).argmax(2)
                indices = torch.cat(((maxidx / s_logits_uw.size(2)).view(s_logits_uw.size(0), s_logits_uw.size(1),1).int(),\
                    (maxidx % s_logits_uw.size(3)).view(s_logits_uw.size(0), s_logits_uw.size(1),1).int()), dim=2)
                hard_pseudo_label = np.zeros((s_logits_uw.size(0),s_logits_uw.size(1), s_logits_uw.size(2), s_logits_uw.size(3)))
                
                for batch in range(s_logits_uw.size(0)):
                    for i in range(s_logits_uw.size(1)):
            ...
            s_loss_u = criterion(hard_pseudo_label, s_logits_us)
            # s_loss_u = criterion(s_logits_uw, s_logits_us)
            s_loss = criterion(s_logits_l, pseudo_label) + s_loss_u
...
def main():
    ...
    output = student_model(dummy_input)  # Warm-up the model
    benchmark_result = benchmark.Timer(
        stmt='model(dummy_input)',
        globals={'model': student_model, 'dummy_input': dummy_input}
    ).timeit(100)  # Run the inference 100 times

    # Print the results
    print('Average inference time:', benchmark_result.mean)
