...
def vis(args,config, data_loader, model):
    ...
    with torch.no_grad():
        for i, (inp, target, meta) in enumerate(data_loader):  
            ...
            point_size = 3
            if str(meta['image'][0]) == 'japanesemacaque_128.jpg':
                point_size = 1
            for pt in pts:
                if pt[0] != 0 and pt[1] != 0:
                    cv2.circle(img,(int(pt[0]),int(pt[1])),point_size,(0,255,0),-1)
            preds = preds.tolist()[0]

            
            for num,pred in enumerate(preds):
                cv2.circle(img,(int(pred[0]),int(pred[1])),point_size,(255,0,0),-1)

            cv2.imwrite('images/'+meta['image'][0],img)
            if nme_temp[0] > 0.1:
                cnt += 1
            if nme_temp[0] > 0.08:
                cnt8 += 1
